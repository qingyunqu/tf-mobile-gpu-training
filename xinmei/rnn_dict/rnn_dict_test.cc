...
Session* loadCKPT(std::string path_to_graph, std::string path_to_ckpt) {
	...
	status = session->Run(
        ...
        nullptr);
	if (!status.ok()) {
    	LOGI("Error loading checkpoint from %s: %s\n", path_to_ckpt.c_str(), status.ToString().c_str());
    	return NULL;
	}
}
...
int main(int argc, char** argv) {
	std::string graph_path = "./model_test.ckpt-781370.meta";
	std::string ckpt_path = "./model_test.ckpt-781370";
	std::vector<Flag> flag_list = {
      Flag("graph", &graph_path, "graph file name"),
      Flag("ckpt", &ckpt_path, "ckpt path"),
  	};
